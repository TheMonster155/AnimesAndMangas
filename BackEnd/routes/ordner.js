...
orders.post("/order/create-order", async (req, res, next) => {
  ...
  try {
    ...
    const { name, city, postalCode, country, street } = shippingAddress;
    if (!name || !city || !postalCode || !country || !street) {
...
orders.post("/order/create-order", async (req, res, next) => {
  ...
  try {
    ...
    // Calcolo del prezzo totale e verifica disponibilità
    let totalPrice = 0;
    for (const item of items) {
      if (item.mangaId) {
        const manga = await mangaModel.findById(item.mangaId);
    ...
    const paymentIntent = await stripe.paymentIntents.create({
      amount: Math.round(totalPrice * 100), // Stripe accetta gli importi in centesimi
      currency: "eur",
      automatic_payment_methods: { enabled: true },
    });
